class HomeController < ApplicationController
  ...
  def index
    ...
    # @geo = Geocoder.search('37.214.53.180')[0].data
    @shelters_list = root_url + 'shelters.json'

    @geo['loc'] = @geo['loc'].split(',')
    @geo['latitude'] = @geo['loc'][0]
    @geo['longitude'] = @geo['loc'][1]
    
    @shelters = Shelter
      .select(:id, :title, :verified, :cover, "cities.title AS _city", "countries.title AS _country")
      .joins(:city, :country)
      .where(moderation: true, status: :active)
      .near([@geo['latitude'], @geo['longitude']], 12000, units: :km)
      .limit(4)
